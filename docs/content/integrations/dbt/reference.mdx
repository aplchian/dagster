...
In the Dagster project created by the [`dagster-dbt project scaffold`](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-scaffold) command line interface, we use <PyObject module="dagster_dbt" object="DbtProject" /> to handle this for us:
...
When developing locally, `dagster dev` will cause the manifest to be regenerated whenever the Definitions are loaded:

```shell
dagster dev
```

As part of deploying to production, the `dagster-dbt` cli can be used to handle compiling the manifest.

```shell
dagster-dbt project prepare-for-deployment --file project_dagster/project_dagster/project.py
```
...
<Note>
  ...
  Dagster tag value is set to special sentinel value{" "}
  <code>"__dagster_no_value"</code>. Any dbt tags that don't match Dagster's
  supported tag key format (e.g. they contain unsupported characters) will be
  ignored by default.
</Note>
